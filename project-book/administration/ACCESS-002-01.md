...
| **Version** | 0.1.0 |
| **Statut** | En cours — deux premiers lots applicatifs publiés en PR brouillon |
...
Le présent état documente uniquement les deux premiers lots publiés dans la [PR applicative brouillon #93](https://github.com/karateseremange/AKS-Platform/pull/93). Il ne clôt pas l’incrément.
...
Les contrôles locaux ciblés réalisés sur la tête applicative `4b59108` sont concluants :
...
Ces résultats ne remplacent pas une exécution cumulative réelle dans Apps Script. La dernière référence cumulative effectivement exécutée reste **455/455 tests réussis, 0 échec**. Les six scénarios ACCESS-002-01 sont intégrés à la suite cumulative, mais cette nouvelle suite n’est pas présentée comme exécutée tant qu’une campagne Apps Script réelle ne l’a pas prouvé.
...
- aucune écriture administrative n’est exposée ;
...
1. une API d’écriture réservée à `ACCESS_MANAGE` ;
2. une validation stricte des adresses, dates, périodes, rôles, capacités et périmètres ;
3. une écriture atomique avec verrou, contrôle de concurrence, relecture et préservation du registre courant en cas d’échec ;
4. la protection du dernier gestionnaire actif fondée sur `ACCESS_MANAGE` effectif ;
5. la désactivation et la réactivation sans restauration automatique des anciennes habilitations ;
6. un audit persistant avant/après avec acteur, cible, résultat et corrélation ;
7. les tests ciblés, de compatibilité et la nouvelle suite cumulative réelle.
