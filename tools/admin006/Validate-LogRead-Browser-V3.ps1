...
param(
    ...
    [string] $CampaignRoot = 'D:\AKS\ADMIN-006-LOGREAD-BROWSER-V3',
    [string] $ClaspPackage = '',
...
if ($Mode -ne 'LocalCheck' -and [string]::IsNullOrWhiteSpace($Authorization)) {
    throw 'Separate V3 authorization required. Creating another version is not authorized.'
}
...
if ($Mode -eq 'Restore' -and [string]::IsNullOrWhiteSpace($Session)) {
    throw 'Restore requires the exact protected V3 session.'
}
...
$Arguments = @(
    (Join-Path $PSScriptRoot 'validate-logread-browser-v3.cjs')
    '--mode'
    $Mode
    '--package-run'
    $PackageRun
...
if ($LASTEXITCODE -ne 0) {
    throw 'LOG_READ browser V3 stopped. Preserve the session and use only the reviewed V3 Restore mode.'
}
