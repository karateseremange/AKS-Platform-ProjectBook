...
param(
    ...
    [string] $CampaignRoot = 'D:\AKS\ADMIN-006-LOGREAD-BROWSER-V4',
    [string] $ClaspPackage = '',
...
if ($Mode -ne 'LocalCheck' -and [string]::IsNullOrWhiteSpace($Authorization)) {
    throw 'Separate V4 authorization required. No version or deployment creation is authorized.'
}
...
if ($Mode -eq 'Restore' -and [string]::IsNullOrWhiteSpace($Session)) {
    throw 'Restore requires the exact protected V4 session.'
}
...
$Arguments = @(
    (Join-Path $PSScriptRoot 'validate-logread-browser-v4.cjs')
    '--mode', $Mode
...
if ($LASTEXITCODE -ne 0) {
    throw 'LOG_READ browser V4 stopped. Preserve the session and use only the reviewed V4 Restore mode.'
}
