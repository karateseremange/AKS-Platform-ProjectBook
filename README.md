...
Le deuxième incrément est cadré dans [`INSCRIPTIONS-008`](project-book/modules/inscriptions/INSCRIPTIONS-008.md). Il reste borné à l’extension d’`ACCESS-001` aux six capacités Inscriptions, aux périmètres explicites et à un cycle d’audit injectable en deux temps, avec intention avant commit puis résultat après contrôle, sans registre réel, ressource Google métier, interface ni déploiement.
