...
Le quatrième incrément est cadré dans [`INSCRIPTIONS-010`](project-book/modules/inscriptions/INSCRIPTIONS-010.md), actuellement en revue. Il propose la première persistance Google strictement limitée à une recette isolée : garde d’environnement, schéma technique `Metadata`/`Sequences`/`Commandes`, verrou réel et adaptateurs persistants du journal et des séquences, avec audit commun AKS Core. Il interdit toute donnée nominative, tout objet métier, toute application de lot, interface ou production. Le bilan reste **13/1/2** et aucun nouveau total de tests n’est annoncé avant preuve.
